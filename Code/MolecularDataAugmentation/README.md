...
The datasets include only the SMILES and SELFIES  formats for each molecule. For training hybrid algorithms the SELFIES format can be used (cf. <a href="https://github.com/aspuru-guzik-group/selfies/tree/master/examples/vae_example"> VAE example </a> for an example) thei `fingerprints` of the molecules can also be useful representations (fingerprints are sparse vector representations of molecules cf. Section 4 in the notebook).
