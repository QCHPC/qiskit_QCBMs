...
class QCBM:
    ...
    def train(self, data=None, loss_fcn=None, num_iterations=10,
              num_shots=100, backend=FakeFez()):
        """Train the QCBM."""
        ...
        minimize(
            self.compute_loss,
            ansatz_params,
            args=(self.ansatz_isa, sampler, loss_fcn, self.train_history,
                  self.target_probs),
            method="cobyla",
        ...
        self.plot_compare_model_and_target_probs(x_max, self.target_probs,
                                                 qcbm_probs)

        return self.train_history

    def compute_loss(self, params, ansatz_isa, sampler, loss_fcn,
                     train_history, target_probs):
        ...
        """
        # params is a numpy array, we need a list for the PUB
        pub = (ansatz_isa, [params])
        # Run the sampler job
        job = sampler.run([pub])
        result = job.result()
        ...
        print(f"""Iters. done: {
                  train_history['iterations']}[Current cost: {loss}]
               """)
    ...
    def plot_compare_model_and_target_probs(self, x_max, target_probs,
                                            qcbm_probs):
        """Compare the probabilities obtained with QCBM with the
           actual probability distribution."""

        plt.plot(range(x_max), target_probs, linestyle="-.", label="target")
        plt.bar(range(x_max), qcbm_probs, color="green", alpha=0.5,
                label="qcbm")
...
class MMD:
    ...
    def __init__(self, rbf_sigmas, nparange_hilbert_dim):
        ...
        sq_dists = np.abs(
            nparange_hilbert_dim[:, None] - nparange_hilbert_dim[None, :]) ** 2
        self.K = sum(np.exp(-gamma * sq_dists)
                     for gamma in gammas) / len(rbf_sigmas)
